...
class SQLiteCompleter(Completer):
    ...
    def __init__(self, conn):
        ...
        self.keywords = [
            ...
            'SET', 'DELETE', 'CREATE', 'TABLE', 'INDEX', 'VIEW', 'TRIGGER',
            'DROP', 'ALTER', 'JOIN', 'LEFT', 'RIGHT', 'INNER', 'OUTER', 
            'CROSS', 'NATURAL', 'ON', 'USING', 'GROUP', 'BY', 'ORDER', 
            'HAVING', 'LIMIT', 'OFFSET', 'UNION', 'INTERSECT', 'EXCEPT',
            'AND', 'OR', 'NOT', 'IN', 'LIKE', 'GLOB', 'BETWEEN', 'IS', 
            'NULL', 'TRUE', 'FALSE', 'EXISTS', 'CASE', 'WHEN', 'THEN', 
            'ELSE', 'END', 'AS', 'DISTINCT', 'ALL', 'ASC', 'DESC',
            'PRIMARY', 'KEY', 'FOREIGN', 'REFERENCES', 'UNIQUE', 'CHECK',
            'DEFAULT', 'AUTOINCREMENT', 'IF', 'NOT', 'EXISTS',
            'BEGIN', 'COMMIT', 'ROLLBACK', 'TRANSACTION', 'SAVEPOINT',
            'PRAGMA', 'EXPLAIN', 'ANALYZE', 'VACUUM', 'REINDEX',
            'COUNT', 'SUM', 'AVG', 'MIN', 'MAX', 'TOTAL', 'GROUP_CONCAT',
            'ABS', 'COALESCE', 'IFNULL', 'NULLIF', 'LENGTH', 'LOWER', 
            'UPPER', 'TRIM', 'LTRIM', 'RTRIM', 'SUBSTR', 'REPLACE',
            'DATE', 'TIME', 'DATETIME', 'STRFTIME', 'JULIANDAY',
        ]
        self.dot_commands = [
            '.tables', '.schema', '.read', '.mode', '.quit', '.exit',
            '.help', '.dump', '.import', '.export', '.columns', '.indexes',
            '.describe', '.count', '.sample', '.timer', '.changes',
            '.parameter', '.param',
        ]
...
class SQLiteCLI:
    ...
    def run(self):
        ...
        console.print("[dim]Tip: ↑↓ navegan el texto, Shift+←→ el historial, Ctrl+O fuerza envío[/]\n")
    ...
    def _cmd_parameter(self, args):
        ...
        if subcmd == 'set' and len(args) >= 3:
            ...
            console.print(f"[green]OK[/] :{name} = {repr(value)} ({type(value).__name__})")
